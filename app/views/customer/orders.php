...
document.getElementById('confirmCancelBtn').addEventListener('click', function() {
    ...
    const oldErr = document.getElementById('cancel-error-msg');
    ...
    .then(data => {
        if (data.success) {
            // Thay nội dung modal thành thông báo thành công
            document.querySelector('#cancelModal .modal-body').innerHTML = `
                <div style="text-align:center; padding: 16px 0;">
                    <i class="fas fa-check-circle" style="font-size:48px; color:#059669; margin-bottom:12px; display:block;"></i>
                    <p style="font-weight:600; color:#065f46; margin:0;">${data.message || 'Đã hủy đơn hàng thành công!'}</p>
                </div>`;
            ...
            // Hiện lỗi inline trong modal — không đóng modal, không dùng alert
            const errDiv = document.createElement('div');
            errDiv.id = 'cancel-error-msg';
            errDiv.style.cssText = 'background:#fee2e2; border:1px solid #fca5a5; color:#991b1b; padding:10px 14px; border-radius:8px; margin-top:12px; font-size:14px; text-align:center;';
            errDiv.innerHTML = `<i class="fas fa-exclamation-circle"></i> ${data.message || 'Hủy đơn hàng thất bại'}`;
            document.querySelector('#cancelModal .modal-body').appendChild(errDiv);
            btn.disabled = false;
            btn.innerHTML = 'Đồng ý hủy đơn';
        }
    })
    .catch(error => {
        ...
        errDiv.id = 'cancel-error-msg';
        errDiv.style.cssText = 'background:#fee2e2; border:1px solid #fca5a5; color:#991b1b; padding:10px 14px; border-radius:8px; margin-top:12px; font-size:14px; text-align:center;';
        errDiv.innerHTML = '<i class="fas fa-exclamation-circle"></i> Có lỗi xảy ra, vui lòng thử lại';
        document.querySelector('#cancelModal .modal-body').appendChild(errDiv);
        btn.disabled = false;
        btn.innerHTML = 'Đồng ý hủy đơn';
    });
